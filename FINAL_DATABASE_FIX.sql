-- ============================================
-- FINAL FIX - Remove userId Foreign Key Constraint
-- This is the CRITICAL fix for your session creation issue
-- ============================================

-- The problem: shopping_sessions has a foreign key to users("userId")
-- But the app creates sessions with userId that doesn't exist in users table
-- Solution: Make userId optional by removing the foreign key constraint
...
BEGIN
    ...
    LOOP
        EXECUTE format('ALTER TABLE shopping_sessions DROP CONSTRAINT %I', constraint_name);
        RAISE NOTICE '✅ Dropped constraint: %', constraint_name;
    END LOOP;
END $$;

-- 2. Similarly, remove cartId constraint to carts if needed (keep as text reference)
-- This is optional but makes the app more flexible
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
RAISE NOTICE '=============================================';
RAISE NOTICE '';
RAISE NOTICE 'Now rebuild your app and test:';
RAISE NOTICE '1. Scan QR code (CART_001)';
RAISE NOTICE '2. Add items to cart';
RAISE NOTICE '3. Check cart page - items should appear!';
RAISE NOTICE '';
RAISE NOTICE 'Look for in logs:';
